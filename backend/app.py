document.addEventListener('DOMContentLoaded', function () {
    ...
    async function init() {
        ...
        try {
            ...
            // Filter out products with missing/broken images and zero prices
            products = filterInvalidProducts(products);
    ...
    // Filter out products with missing images or zero prices
    function filterInvalidProducts(products) {
        return products.filter(product => {
            // Check if product has a valid image
            const hasValidImage = product.image_link && 
                product.image_link.trim() !== '' && 
                !product.image_link.includes('placeholder');
            
            // Check if product has a valid price (not zero or empty)
    ...
    // Validate image URL with a preloading check
    ...
    async function displayProducts(products) {
        ...
        // Array to store promises for image validation
        const productDisplayPromises = [];

        products.forEach(product => {
            // Skip products with invalid image URLs or prices right away
            ...
            // Create promise for image validation
            const imagePromise = new Promise(resolve => {
                const img = card.querySelector('.product-image');
                
                img.onload = () => {
                    // Image loaded successfully, show the card
                    card.style.display = '';
                    resolve();
                };
                
                img.onerror = () => {
                    // Image failed to load, remove the card
                    card.remove();
                    resolve();
                };
            });

            productDisplayPromises.push(imagePromise);
        });

        // Wait for all images to be processed
        await Promise.all(productDisplayPromises);

        // Check if we need to show no results message after filtering out bad images
